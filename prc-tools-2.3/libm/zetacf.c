...
#ifdef ANSIC
float sin(float), floor(float), gamma(float), pow(float, float);
float exp(float);
float polevl(), p1evl();
#else
float sin(), floor(), gamma(), pow(), exp();
float polevl(), p1evl();
#endif

#ifdef ANSIC
float zetac(float xx)
#else
float zetac(xx)
double xx;
#endif
...
if( x < 0.0f )
	...
	s = 1.0f - x;
	w = zetac( s );
	b = sin(PIO2F*x) * pow(2.0f*PIF, x) * gamma(s) * (1.0f + w) / PIF;
	return(b - 1.0f);
	}
